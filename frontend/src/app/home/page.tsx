...
function fmtDateTime(iso: string) {
  ...
  return d.toLocaleString('pt-BR', { weekday: 'long', day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit', timeZone: 'UTC' })
    .replace('.', '')
}
  ...
  return (
    <main className="min-h-screen bg-[#1E2030] text-white pb-28">
      <div className="mx-auto max-w-sm px-4 pt-6">
        ...
        {loading && (
          <div className="bg-white/10 h-24 rounded-2xl animate-pulse" />
        )}

        {!loading && error && (
          <div className="bg-red-100 text-red-700 px-4 py-2 rounded-2xl">{error}</div>
        )}
        ...
        {!loading && !error && (
          <>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="font-semibold">
                      {summary!.active_plan!.type === 'QUINZENAL' ? 'Quinzenal, 40min'
                       : summary!.active_plan!.type === 'SEMANAL' ? 'Semanal, 40min'
                       : 'Avulso, 40min'}
                    </div>
                  </div>
                </div>
            ...
            {hasNext && (
                <h3 className="mt-4 mb-2 text-white/90 font-semibold">
                    Próximas Consultas com {therapistName}
                </h3>
            )}
