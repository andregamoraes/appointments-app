	...
	return (
		<main className='max-w-sm mx-auto p-4'>
			...
			<div className='bg-white rounded-2xl shadow p-2'>
				<Calendar onChange={chooseDate} value={date} minDate={new Date()} locale='pt-BR' tileDisabled={({ date }) => [0, 6].includes(date.getDay())} />
			</div>
			...
			{!loading && !!times.length && (
				<div className='grid grid-cols-4 gap-3 mt-4'>
					{times.map(t => (
						<button key={t} onClick={() => chooseTime(t)} className='px-3 py-2 bg-green-200 rounded cursor-pointer hover:bg-green-300 text-sm font-medium'>
							{t}
						</button>
					))}
				</div>
			...
			{!loading && !error && !times.length && <p className='mt-4 text-gray-500'>Sem horários para o dia selecionado.</p>}

			<div className='mt-6'>
				<button onClick={() => router.back()} className='w-full bg-gray text-gray-800 py-3 rounded-2xl font-semibold hover:bg-gray-200 transition cursor-pointer'>
					Voltar
				</button>
			</div>
		</main>
