	...
	return (
		<main className='min-h-screen bg-[#1E2030] text-white pb-28'>
			<div className='mx-auto max-w-sm px-4 pt-6'>
						...
						return (
							<button
								...
								<div className='flex items-start justify-between'>
									<div>
										<p className='text-[11px] font-semibold tracking-wide text-gray-400'>{p.title}</p>
										<p className='text-gray-900 font-semibold mt-1'>{p.subtitle}</p>
									</div>

									<div className='text-right leading-none'>
										<p className='text-pink-600 font-bold'>{p.price}</p>
										{!!p.per && <span className='text-[10px] text-gray-400'>{p.per}</span>}
									</div>
								</div>
							</button>
						);
				...
				<div className='mt-6'>
					<button onClick={() => router.back()} className='w-full bg-white/90 text-gray-800 py-3 rounded-2xl font-semibold hover:bg-white cursor-pointer transition'>
						Voltar
					</button>
				</div>
			</div>
